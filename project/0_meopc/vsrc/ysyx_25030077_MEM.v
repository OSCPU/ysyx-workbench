module ysyx_25030077_MEM(
  ...
  wire  [31:0] valid = {31'b0, io_ar_valid};
  ...
  reg [31:0] mem_data_Reg; // @[mem.scala 20:29]
  reg  validReg; // @[mem.scala 21:25]
  ...
  wire  canAccept = _canAccept_T[0]; // @[mem.scala 22:28]
  ...
  wire [31:0] read_data = mem_data_read({29'h0,io_r_mask}, io_rs1, io_rs2, io_imm);
  reg [2:0] delayCnt; // @[mem.scala 25:25]
  wire  _startDelay_T = io_ar_valid & canAccept; // @[mem.scala 27:29]
  wire  startDelay = io_ar_valid & canAccept & io_r_valid; // @[mem.scala 27:42]
  wire [2:0] _delayCnt_T = {delayCnt_prng_io_out_2,delayCnt_prng_io_out_1,delayCnt_prng_io_out_0}; // @[PRNG.scala 95:17]
  wire [2:0] _delayCnt_T_3 = delayCnt - 3'h1; // @[mem.scala 30:46]
  wire  _io_r_valid_T = delayCnt == 3'h0; // @[mem.scala 33:39]
  wire  _validReg_T_4 = io_b_ready & io_r__ready & _io_r_valid_T ? 1'h0 : validReg; // @[mem.scala 39:18]
  ...
  assign io_ar_ready = _canAccept_T[0]; // @[mem.scala 22:28]
  assign io_aw_ready = _canAccept_T[0]; // @[mem.scala 22:28]
  assign io_w_ready = _canAccept_T[0]; // @[mem.scala 22:28]
  assign io_mem_data = mem_data_Reg; // @[mem.scala 36:15]
  assign io_r__valid = validReg & delayCnt == 3'h0; // @[mem.scala 33:26]
  assign io_b_valid = validReg & _io_r_valid_T; // @[mem.scala 34:26]
  ...
  always @(posedge clock) begin
    if (reset) begin // @[mem.scala 20:29]
      mem_data_Reg <= 32'h0; // @[mem.scala 20:29]
    end else if (io_r_valid & io_ar_valid & canAccept) begin // @[mem.scala 41:22]
      mem_data_Reg <= read_data;
    end
    if (reset) begin // @[mem.scala 21:25]
      validReg <= 1'h0; // @[mem.scala 21:25]
    end else begin
      validReg <= _startDelay_T | _validReg_T_4; // @[mem.scala 38:12]
    end
    if (reset) begin // @[mem.scala 25:25]
      delayCnt <= 3'h0; // @[mem.scala 25:25]
    end else if (startDelay) begin // @[mem.scala 29:18]
      delayCnt <= _delayCnt_T;
    end else if (delayCnt != 3'h0) begin // @[mem.scala 30:18]
