...
static int decode_exec(Decode *s) {
  ...
  #define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
    ...
    INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw,C,   printf("csrrw %x\n", imm); int t = SR(imm); SR(imm) = src1;R(rd) = t;);
    INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs,C,   printf("csrrs %x\n", imm); int t = SR(imm); SR(imm) = src1 | t;R(rd) = t;);
