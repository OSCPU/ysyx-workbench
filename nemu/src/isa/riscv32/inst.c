...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(rd) = (src1 < src2) ? 1U: 0U);
  ...
  // load
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = Mr(src1 + imm, 1) | data_sext(Mr(src1 + imm, 1), 1));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = Mr(src1 + imm, 2) | data_sext(Mr(src1 + imm, 2), 2));
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , M, R(rd) = (src2 == 0) ? src1 : ((int32_t)src1 % (int32_t)src2));
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = CSR(isa_csr_str2addr("mepc")) + 4);
...
uint32_t highest_bit_mask(uint32_t data) {
    int highestNonZeroBit = -1;  // 初始化为-1，表示未找到非零位
  ...
  if(highestNonZeroBit >= 0 && data >> highestNonZeroBit) {
    uint32_t ret = 0;
    for (int i=32; i>= highestNonZeroBit; i--) {
      ret |= 1 << i;
    }
    return ret;
  }
