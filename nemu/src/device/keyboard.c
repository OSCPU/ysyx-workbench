...
/* nemu中通过SDL读取按键信息，然后调用send_key()进行处理key信息，再调用key_enqueue()（一个循环数组，将am_scancode存储进入）
   对于运行的程序，每次都会调用i8042_data_io_handler，对队列中的扫描码进行读取与处理，具体就是调用key_dequeue()
   该函数会将处理后的key信息放入i8042_data_port_base地址中
 */
