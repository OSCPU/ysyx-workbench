...
void init_elf(const char* elf_file, size_t global_offset){
    ...
    for (int i = 0; i < symbol_table_total_size / symbol_table_entry_size; ++i){    //sh_size/sh_entsize求出条文数目
        ...
        switch(ELF32_ST_TYPE(symbol_section_entry.st_info)){
            ...
            append(function_name, symbol_section_entry.st_value, symbol_section_entry.st_size);  //st_value是函数的起始地址，st_size是函数的大小 append函数将函数名，函数起始地址，函数大小存入elf_funcs数组中
            break;
        }
    }
    printf("====== Symbol Table ======\n");
    for (int i = 0; i < end; ++i){
        FuncInfo *info = &elf_funcs[i];
